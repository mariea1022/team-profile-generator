function generateTeamProfile(data) {
    // first part 
    let html = `<html lang="en">
    <head>
        <meta charset="UTF-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
        <title>Team Profile Generator</title>
    </head>
    <body>`; 

    for(let i = 0; i<data.length; i++) {
        var employeeType = data[i];
        if(employeeType.getRole() === "Manager") {
            // html+= 
            html += `<div class="card" style="width: 18rem;">
            <div class="card-header">${data.teamManagerName}</div>
                <ul class="list-group list-group-flush">
                    <li class="list-group-item">ID: ${data.teamManagerID}</li>
                    <li class="list-group-item">Email: ${data.teamManagerEmail}</li>
                    <li class="list-group-item">Office number: ${data.teamManagerOfficeNumber}</li>
                </ul>
            </div>`
        } else if(employeeType.getRole() === "Intern") {
            // html+= 
            html += `<div class="card" style="width: 18rem;">
            <div class="card-header">${data.teamManagerName}</div>
                <ul class="list-group list-group-flush">
                    <li class="list-group-item">ID: ${data.teamManagerID}</li>
                    <li class="list-group-item">Email: ${data.teamManagerEmail}</li>
                    <li class="list-group-item">Office number: ${data.teamManagerOfficeNumber}</li>
                </ul>
            </div>`
        }
        //...
    }

    // the rest of the html
    html += `<script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/popper.js@1.12.9/dist/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
    </body>
    </html>
    `; 

    return html;

    

    

}

module.exports = generateTeamProfile;